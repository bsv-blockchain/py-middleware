...
class BSVPaymentFlowTester:
    ...
    def __init__(self):
        ...
        self.payment_middleware = create_payment_middleware(
            calculate_request_price=self.calculate_test_price,
    ...
    def create_payment_header(self, satoshis: int, endpoint: str = "/premium/", identity_key: str = "033f5aed5f6cfbafaf94570c8cde0c0a6e2b5fb0e07ca40ce1d6f6bdfde1e5b9b8") -> str:
        ...
        derivation_prefix = f"{endpoint}:{identity_key[:20]}..." if identity_key != 'unknown' else endpoint
    ...
    def get_payment_test_scenarios(self) -> List[PaymentTestScenario]:
        ...
        return [
            ...
            PaymentTestScenario(
                ...
                required_payment=1000,
                headers={
                    'x-bsv-payment': None  # Will be set in test, but no auth
                },
    ...
    def test_payment_scenario(self, scenario: PaymentTestScenario) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'x-bsv-payment' in headers and headers['x-bsv-payment'] is None:
                identity_key = headers.get('x-bsv-auth-identity-key', '033f5aed5f6cfbafaf94570c8cde0c0a6e2b5fb0e07ca40ce1d6f6bdfde1e5b9b8')
                if scenario.name == "Valid payment - overpayment":
                    headers['x-bsv-payment'] = self.create_payment_header(scenario.required_payment + 500, scenario.endpoint, identity_key)
                elif scenario.name == "Insufficient payment":
                    headers['x-bsv-payment'] = self.create_payment_header(scenario.required_payment - 500, scenario.endpoint, identity_key)
                elif scenario.name == "Payment without authentication":
                    headers['x-bsv-payment'] = self.create_payment_header(scenario.required_payment, scenario.endpoint, identity_key)
                else:
                    headers['x-bsv-payment'] = self.create_payment_header(scenario.required_payment, scenario.endpoint, identity_key)
            ...
            def mock_get_response(req):
                return JsonResponse({
                    ...
                    'path': req.path,
                    'payment_processed': hasattr(req, 'bsv_payment') and req.bsv_payment.accepted if hasattr(req, 'bsv_payment') else False
                })
            ...
            payments_before = len(self.mock_wallet.get_processed_payments())
            payment_processed = hasattr(request, 'bsv_payment') and getattr(request.bsv_payment, 'accepted', False)
            ...
            print(f"   Payment processed: {payment_processed} (expected: {scenario.should_process_payment})")
    ...
    def test_payment_calculation(self) -> Dict[str, Any]:
        ...
        for path, expected_price in test_paths:
            try:
                ...
                print(f"   {status} {path}: {calculated_price} satoshis (expected {expected_price})")
                
            except Exception as e:
                print(f"   ❌ Error calculating price for {path}: {str(e)}")
                results.append({
                    'path': path,
                    'error': str(e),
                    'success': False
    ...
    def run_all_payment_tests(self) -> Dict[str, Any]:
        ...
        calc_passed = sum(1 for t in calculation_results.get('calculation_tests', []) if t.get('success', False))
        ...
        print("\n" + "=" * 60)
        print(f"📊 Payment Flow Summary: {passed_tests}/{total_tests} tests passed ({pass_rate:.1f}%)")
